...
def get_users(
    ...
    if identifier:
        users = session.execute(request).unique().scalars().all()
    else:
        users1 = session.execute(request).unique().scalar_one()
...
def build_post_out(posts: Sequence[tuple[Post, str]]) -> list[PostOut]:
    posts_out = []
...
def get_posts(
    ...
    if user_id is None:
        base_request = base_request.join(User)
    else:
        base_request = base_request.where(Post.user_id == user_id).join(User)
    posts = session.execute(base_request).scalars().all()
    return build_post_out(posts)
